    ...
    if($('.'+barcode).length) {
      ...
      $('.'+barcode).each(function() {
        if(valid == 0 && qty > 0) {
          ...
          if(diff > 0) {
            let buffer = parseDefaultFloat($('#buffer-'+id).val(), 0);
          ...
          $('.receive-item').removeClass('highlight');
          $('#receive-item-'+id).addClass('highlight');
          $('#receive-item-'+id).prependTo($('#incomplete-box'));

          if(balance == 0) {
            $('#receive-item-'+id).removeClass('unvalid').addClass('valid');
          }
        }
      });
