...
function getZoneTo() {
	...
	if( barcode.length > 0 ){
		//---	คลังปลายทาง
		var warehouse_code = $("#warehouse-code").val();
...
$("#barcode-item-to").keyup(function(e) {
    if( e.keyCode == 13 ) {
		...
		//---	ไอดีโซนปลายทาง
		let zone_code	= $("#to_zone_code").val().trim();
		...
		let qty = parseDefaultInt($("#qty-to").val(), 0);

		let curQty	= parseDefaultInt($("#qty-"+barcode).val(), 0);
