...
class Sales_channels_details extends PS_Controller
{
  ...
  public function do_export()
  {
    ...
    $ds = array(
      ...
      'item_from' => $pdFrom,
      'item_to' => $pdTo,
      'from_reference' => $refCodeFrom,
      'to_reference' => $refCodeTo,
      'from_date' => $fromDate,
      'to_date' => $toDate
    );
    ...
    $this->excel->getActiveSheet()->setCellValue('G5', 'ที่อยู่');
    $this->excel->getActiveSheet()->setCellValue('H5', 'ตำบล/แขวง');
    $this->excel->getActiveSheet()->setCellValue('I5', 'อำเภอ/เขต');
    ...
    $this->excel->getActiveSheet()->setCellValue('T5', 'สถานะ');
    $this->excel->getActiveSheet()->setCellValue('U5', 'ขนส่ง');
    ...
    if(!empty($result))
    {
      ...
      foreach($result as $rs)
      {
        ...
        //--- สถานะออเดอร์
        $this->excel->getActiveSheet()->setCellValue('T'.$row, $rs->state);

        //--- ขนส่ง
        $this->excel->getActiveSheet()->setCellValue('U'.$row, $rs->sender);
