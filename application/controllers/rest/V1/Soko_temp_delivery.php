...
class Soko_temp_delivery extends PS_Controller
{
	public $menu_code = 'SOKOOB';
	public $menu_group_code = 'SOKOJUNG';
  public $menu_sub_group_code = '';
	public $title = 'SOKOCHAN Delivery Temp';
  public $filter;

  public function __construct()
  {
    parent::__construct();
    $this->home = base_url().'rest/V1/soko_temp_delivery';
  ...
  public function index()
  {
    $filter = array(
      ...
      'status' => get_filter('status', 'do_status', 'all'),
		...
		$rows     = $this->soko_temp_order_model->count_rows($filter);
		//--- ส่งตัวแปรเข้าไป 4 ตัว base_url ,  total_row , perpage = 20, segment = 3
		$init	    = pagination_config($this->home.'/index/', $rows, $perpage, $segment);
		$orders   = $this->soko_temp_order_model->get_list($filter, $perpage, $this->uri->segment($segment));

    $filter['orders'] = $orders;

		$this->pagination->initialize($init);

    $this->load->view('rest/V1/sokojung/delivery/temp_delivery_list', $filter);
  }
	  ...
	  {
			$order = $this->soko_temp_order_model->get($id);

	    $ds['details'] = $this->soko_temp_order_model->get_details($id);
			$ds['code'] = !empty($order) ? $order->code : NULL;
	    $this->load->view('rest/V1/sokojung/delivery/temp_delivery_detail', $ds);
	  }
	...
	public function close_temp()
	{
		...
		if(! $this->soko_temp_order_model->update($id, $arr))
		{
			$sc = FALSE;
	...
	public function delete($id) {
		$sc = TRUE;
		$rs = $this->soko_temp_order_model->delete($id);
		if(! $rs)
		{
			$sc = FALSE;
