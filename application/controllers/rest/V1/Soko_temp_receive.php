...
class Soko_temp_receive extends PS_Controller
{
	public $menu_code = 'SOKOIB';
	public $menu_group_code = 'SOKOJUNG';
  public $menu_sub_group_code = '';
	public $title = 'Soko Receive Temp';
  public $filter;

  public function __construct()
  {
    parent::__construct();
    $this->home = base_url().'rest/V1/soko_temp_receive';
  ...
  public function index()
  {
    $filter = array(
      ...
      'status' => get_filter('status', 'receive_status', 'all'),
		...
		$rows     = $this->soko_temp_receive_model->count_rows($filter);
		//--- ส่งตัวแปรเข้าไป 4 ตัว base_url ,  total_row , perpage = 20, segment = 3
		$init	    = pagination_config($this->home.'/index/', $rows, $perpage, $segment);
		$orders   = $this->soko_temp_receive_model->get_list($filter, $perpage, $this->uri->segment($segment));

    $filter['orders'] = $orders;

		$this->pagination->initialize($init);

    $this->load->view('rest/V1/sokojung/receive/temp_receive_list', $filter);
  }
	  ...
	  {
			$order = $this->soko_temp_receive_model->get($id);

	    $ds['details'] = $this->soko_temp_receive_model->get_details($id);
			$ds['code'] = !empty($order) ? $order->code : NULL;
	    $this->load->view('rest/V1/sokojung/receive/temp_receive_detail', $ds);
	  }
	...
	{
		$sc = TRUE;
		$rs = $this->soko_temp_receive_model->delete($id);
		if(! $rs)
		{
			$sc = FALSE;
	...
	public function close_temp($id)
	{
		...
		if(! $this->soko_temp_receive_model->update($id, $arr))
		{
			$sc = FALSE;
