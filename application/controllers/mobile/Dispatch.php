...
class Dispatch extends PS_Controller
{
  ...
  {
    parent::__construct();
    $this->home = base_url().'mobile/dispatch';
  ...
  public function index()
  {
    ...
    else
    {
      ...
      $this->load->view('mobile/dispatch/dispatch_list', $filter);
    }
  }
  ...
  {
    $this->load->view('mobile/dispatch/dispatch_add');
  }
  ...
  public function add()
  {
    ...
    if($this->pm->can_add)
    {
      if( ! empty($ds))
      {
        ...
        $channels_name = empty($ds->channels_name) ? 'ไม่ระบุ' : $ds->channels_name;
    ...
    if( ! empty($order_code) && ! empty($code))
    {
      ...
      if( ! empty($order))
      {
        if($order->state == 8)
        {
          ...
          if( ! empty($order->reference) && ($order->channels_code == $tiktok OR $order->channels_code == $shopee OR $order->channels_code == $lazada) && is_true(getConfig('WRX_API')))
          {
  ...
  public function edit($code)
  {
    if($this->pm->can_add OR $this->pm->can_edit)
    {
      ...
      if( ! empty($doc))
      {
        ...
        $this->load->view('mobile/dispatch/dispatch_edit', $ds);
      }
      else
      {
        $this->error_page();
  ...
  public function view_pending_order($code)
  {
    ...
    if( ! empty($doc))
    {
      ...
      if($this->agent->is_mobile())
      {
        ...
        $this->load->view('mobile/dispatch/mobile/view_pending_mobile', $ds);
      }
      else
      {
        $this->load->view('mobile/dispatch/dispatch_view_pending', $ds);
      }
    }
    else
    {
  ...
  public function view_detail($code)
  {
    ...
    if( ! empty($doc))
    {
      ...
      $this->load->view('mobile/dispatch/dispatch_view_detail', $ds);
    }
    else
    {
      $this->error_page();
