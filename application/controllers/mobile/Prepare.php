...
class Prepare extends PS_Controller
{
  ...
  {
    parent::__construct();
    $this->home = base_url().'mobile/prepare';
  ...
  public function index()
  {
    ...
    else
    {
      ...
      $this->load->view('mobile/prepare/prepare_list', $filter);
    }
  }
  ...
  public function view_process()
  {
    ...
    else
    {
      //--- แสดงผลกี่รายการต่อหน้า
  		...
  		$init	= mobile_pagination_config($this->home.'/view_process/', $rows, $perpage, $segment);
      ...
      $this->load->view('mobile/prepare/process_list', $filter);
    }
  }
  ...
  public function process($code)
  {
    ...
    if( ! empty($order))
    {
      ...
      if( ! $is_cancel)
      {
        ...
        $this->load->view('mobile/prepare/prepare_process', $ds);
      }
      else
      {
        $this->orders_model->update($code, ['is_cancled' => 1]);
        $this->load->view('mobile/prepare/order_cancelled', ['order' => $order]);
      }
    }
    else
    {
