...
class Product_model extends PS_Controller
{
  ...
  public function add()
  {
    ...
    {
      if($this->product_model_model->is_exists($code))
      {
        $sc = FALSE;
      ...
      if($sc === TRUE)
      {
        ...
        if( ! $this->product_model_model->add($ds))
        {
          set_error('insert');
        }
      }
  ...
  {
    $rs = $this->product_model_model->get_by_id($id);

    $this->load->view('masters/product_model/product_model_edit', $rs);
  }
  ...
  public function update()
  {
    ...
    if( ! empty($code) && ! empty($name))
    {
      if($this->product_model_model->is_exists($name, $id))
      {
        $sc = FALSE;
        set_error('exists', $name);
      }
      ...
      if($sc === TRUE)
      {
        if( ! $this->product_model_model->update_by_id($id, ['name' => trim($name)]))
        {
          $sc = FALSE;
          set_error('update');
        }
  ...
  public function delete()
  {
    ...
    if( ! empty($id))
    {
      if($this->pm->can_delete)
      {
        if( ! empty($code))
        {
          if( ! $this->product_model_model->delete_by_id($id))
          {
            set_error('delete');
          }
        }
