...
class Customer_class extends PS_Controller
{
  ...
  public function index()
  {
    ...
    else
    {
      ...
      $filter['data'] = $this->customer_class_model->get_list($filter, $perpage, $this->uri->segment($segment));
  ...
  public function edit($code)
  {
    if($this->pm->can_edit)
    {
      $ds = $this->customer_class_model->get($code);

      if( ! empty($ds))
      {
        $this->load->view('masters/customer_class/customer_class_edit', ['ds' => $ds]);
      }
      else
      {
        $this->page_error();
  ...
  public function update()
  {
    ...
    if($this->pm->can_edit)
    {
      ...
      if( ! empty($ds) && ! empty($ds->code) && ! empty($ds->name))
      {
        if($this->customer_class_model->is_exists_name($ds->name, $ds->code))
        {
          $sc = FALSE;
          set_error('exists', $ds->name);
        }
        ...
        if($sc === TRUE)
        {
          ...
          if( ! $this->customer_class_model->update($ds->code, $arr))
          {
            $sc = FALSE;
            set_error('update');
          }
  ...
  public function delete()
  {
    ...
    if($this->pm->can_delete)
    {
      ...
      if( ! empty($code))
      {
        if($this->customer_class_model->has_transection($code))
        {
          $sc = FALSE;
