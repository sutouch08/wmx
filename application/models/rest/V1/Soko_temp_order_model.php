...
class Soko_temp_order_model extends CI_Model
{
	private $tb = "soko_temp_order";  //---- table nmae
	private $td = "soko_temp_order_detail"; //---- table name
	...
	public function delete($id)
	{
		...
		$rd = $this->wms->where('id_order', $id)->delete($this->td);
		$rs = $this->wms->where('id', $id)->delete($this->tb);
		$rt = $this->wms->where('id_order', $id)->delete('soko_tracking_detail');
	...
	public function add_tracking(array $ds = array())
	{
		if( ! empty($ds))
		{
			return $this->wms->insert('soko_tracking_detail', $ds);
		}
