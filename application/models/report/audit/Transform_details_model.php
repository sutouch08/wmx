<?php
class Transform_details_model extends CI_Model
{
  ...
  public function get_data(array $ds = array())
	{
		...
		{
			$this->db
			->select('od.order_code, od.original_code, od.product_code')
			->select('od.sold_qty AS qty, od.receive_qty AS receive, (od.sold_qty - od.receive_qty) AS balance, od.date_update')
