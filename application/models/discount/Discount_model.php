  ...
  public function __construct()
  {
	...
	public function get_rule_list(object $pd, object $cs, $price, $qty, $payment_code, $channels_code, $date, array $policy_ids = array())
	{
		if( ! empty($policy_ids))
		{
			...
			->join('discount_rule_product_style AS pm', 'r.id = pm.id_rule', 'left')
			->join('discount_rule_product_group AS pg', 'r.id = pg.id_rule', 'left')
			->join('discount_rule_product_sub_group AS ps', 'r.id = ps.id_rule', 'left')
			->join('discount_rule_product_kind AS pk', 'r.id = pk.id_rule', 'left')
			->join('discount_rule_product_category AS pc', 'r.id = pc.id_rule', 'left')
			...
			->group_start()->where('pm.style_code IS NULL', NULL, FALSE)->or_where('pm.style_code', $pd->style_code)->group_end()
			->group_start()->where('pg.group_code IS NULL', NULL, FALSE)->or_where('pg.group_code', $pd->group_code)->group_end()
			->group_start()->where('ps.sub_group_code IS NULL', NULL, FALSE)->or_where('ps.sub_group_code', $pd->sub_group_code)->group_end()
			->group_start()->where('pk.kind_code IS NULL', NULL, FALSE)->or_where('pk.kind_code', $pd->kind_code)->group_end()
			->group_start()->where('pc.category_code IS NULL', NULL, FALSE)->or_where('pc.category_code', $pd->category_code)->group_end()
