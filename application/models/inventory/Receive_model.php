...
class Receive_model extends CI_Model
{
  ...
  private $log = "receive_logs"; //--- inbound logs
  ...
  public function get_unvalid_details($receive_id)
  {
    $rs = $this->db->where('receive_id', $receive_id)->where('valid', 0)->get($this->td);
  ...
  public function get_valid_details($receive_id)
  {
    $rs = $this->db->where('receive_id', $receive_id)->where('valid', 1)->get($this->td);
  ...
  public function get_list(array $ds = array(), $perpage = 20, $offset = 0, $status = 'P')
  {
  ...
  public function count_rows(array $ds = array(), $status = 'P')
  {
    $this->db->where('status', $status);
  ...
  public function add_logs($code, $action, $user)
  {
    $ds = array(
      'code' => $code,
      'action' => $action,
