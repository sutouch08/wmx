...
class Consignment_order_model extends CI_Model
{
  ...
  {
    $qr  = "UPDATE consignment_order ";
    $qr .= "SET DocTotal = (SELECT SUM(amount) FROM consignment_order_detail WHERE consign_code = '{$code}'), ";
    $qr .= "TotalQty = (SELECT SUM(qty) FROM consignment_order_detail WHERE consign_code = '{$code}') ";
    $qr .= "WHERE code = '{$code}'";
